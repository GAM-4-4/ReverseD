	...
	if (JelSmije(ocjena)) {
		...
		while (getline(Uèitelji, RedT)) {
			...
			if ((ime == ucitelj1) && nijeJoš) {
				...
				//Potvrda u unosu
				m_ocjena->~wxTextCtrl();
				m_ocjena = new wxTextCtrl(this, wxID_ANY, "Ocjena uspješno unesena", wxPoint(50, 630), wxSize(375, 50));
			}
		...
		//Poruka za fail
		m_ocjena->~wxTextCtrl();
		m_ocjena = new wxTextCtrl(this, wxID_ANY, "Ocjena nije od 1 do 5 ili format nije toèan", wxPoint(50, 630), wxSize(375, 50));
	}
...
cUcitelj::cUcitelj(wxMDIParentFrame* parent, wxString sName) : wxMDIChildFrame(parent, wxID_ANY, sName) {
	...
	m_ocjena = new wxTextCtrl(this, wxID_ANY, "Unesi ocjenu od 1 do 5 u formatu 'ocjena,redniBrojKriterija' (npr. 3 iz Odnosa prema radu je 3,1)", wxPoint(50, 630), wxSize(375, 50));
